...
namespace Reliant.RenuOil.DAL.EntityClasses
{
		...
		// __LLBLGENPRO_USER_CODE_REGION_END	
	{
		...
		private Reliant.RenuOil.DAL.CollectionClasses.EmployeeCollection _employeeCollectionViaLoad;
		private bool	_alwaysFetchEmployeeCollectionViaLoad, _alreadyFetchedEmployeeCollectionViaLoad;
		...
		public StatusEntity(System.Int32 statusId)
		{
		...
		public StatusEntity(System.Int32 statusId, IPrefetchPath prefetchPathToUse)
		{
		...
		public StatusEntity(System.Int32 statusId, IValidator validator)
		{
		...
		protected override void PostReadXmlFixups()
		{
			...
			_alreadyFetchedEmployeeCollectionViaLoad = (_employeeCollectionViaLoad.Count > 0);
		...
		public override void GetObjectData(SerializationInfo info, StreamingContext context)
		{
			...
			info.AddValue("_employeeCollectionViaLoad", (!this.MarkedForDeletion?_employeeCollectionViaLoad:null));
			info.AddValue("_alwaysFetchEmployeeCollectionViaLoad", _alwaysFetchEmployeeCollectionViaLoad);
			info.AddValue("_alreadyFetchedEmployeeCollectionViaLoad", _alreadyFetchedEmployeeCollectionViaLoad);
		...
		public override void SetRelatedEntityProperty(string propertyName, IEntity entity)
		{
			switch(propertyName)
			{
				...
				case "Load":
					...
					break;
				case "EmployeeCollectionViaLoad":
					_alreadyFetchedEmployeeCollectionViaLoad = true;
					if(entity!=null)
					{
		...
		public bool FetchUsingPK(System.Int32 statusId)
		{
		...
		/// <param name="prefetchPathToUse">the PrefetchPath which defines the graph of objects to fetch as well</param>
		/// <returns>True if succeeded, false otherwise.</returns>
		public bool FetchUsingPK(System.Int32 statusId, IPrefetchPath prefetchPathToUse)
		{
		...
		/// <returns>True if succeeded, false otherwise.</returns>
		public bool FetchUsingPK(System.Int32 statusId, IPrefetchPath prefetchPathToUse, Context contextToUse)
		{
			return Fetch(statusId, prefetchPathToUse, contextToUse, null);
		}
		...
		/// <returns>True if succeeded, false otherwise.</returns>
		public bool FetchUsingPK(System.Int32 statusId, IPrefetchPath prefetchPathToUse, Context contextToUse, ExcludeIncludeFieldsList excludedIncludedFields)
		{
		...
		public virtual Reliant.RenuOil.DAL.CollectionClasses.LoadCollection GetMultiLoad(bool forceFetch, IEntityFactory entityFactoryToUse, IPredicateExpression filter)
		{
 			if( ( !_alreadyFetchedLoad || forceFetch || _alwaysFetchLoad) && !base.IsSerializing && !base.IsDeserializing && !base.InDesignMode)
			{
				...
				_load.GetMultiManyToOne(null, this, filter);
		...
		public Reliant.RenuOil.DAL.CollectionClasses.EmployeeCollection GetMultiEmployeeCollectionViaLoad(bool forceFetch)
		{
			return GetMultiEmployeeCollectionViaLoad(forceFetch, _employeeCollectionViaLoad.EntityFactoryToUse);
		}
		...
		/// <returns>Filled collection with all related entities of the type constructed by the passed in entity factory</returns>
		public Reliant.RenuOil.DAL.CollectionClasses.EmployeeCollection GetMultiEmployeeCollectionViaLoad(bool forceFetch, IEntityFactory entityFactoryToUse)
		{
 			if( ( !_alreadyFetchedEmployeeCollectionViaLoad || forceFetch || _alwaysFetchEmployeeCollectionViaLoad) && !base.IsSerializing && !base.IsDeserializing && !base.InDesignMode)
			{
				if(base.ParticipatesInTransaction)
				{
					if(!_employeeCollectionViaLoad.ParticipatesInTransaction)
					{
						base.Transaction.Add(_employeeCollectionViaLoad);
					}
				}
				IPredicateExpression filter = new PredicateExpression();
				filter.Add(new FieldCompareValuePredicate(StatusFields.StatusId, ComparisonOperator.Equal, this.StatusId, "StatusEntity__"));
				_employeeCollectionViaLoad.SuppressClearInGetMulti=!forceFetch;
				if(entityFactoryToUse!=null)
				{
					_employeeCollectionViaLoad.EntityFactoryToUse = entityFactoryToUse;
				}
				_employeeCollectionViaLoad.GetMulti(filter, GetRelationsForField("EmployeeCollectionViaLoad"));
				_employeeCollectionViaLoad.SuppressClearInGetMulti=false;
				_alreadyFetchedEmployeeCollectionViaLoad = true;
			}
			return _employeeCollectionViaLoad;
		}

		/// <summary> Sets the collection parameters for the collection for 'EmployeeCollectionViaLoad'. These settings will be taken into account
		/// when the property EmployeeCollectionViaLoad is requested or GetMultiEmployeeCollectionViaLoad is called.</summary>
		/// <param name="maxNumberOfItemsToReturn"> The maximum number of items to return. When set to 0, this parameter is ignored</param>
		/// <param name="sortClauses">The order by specifications for the sorting of the resultset. When not specified (null), no sorting is applied.</param>
		public virtual void SetCollectionParametersEmployeeCollectionViaLoad(long maxNumberOfItemsToReturn, ISortExpression sortClauses)
		{
			_employeeCollectionViaLoad.SortClauses=sortClauses;
			_employeeCollectionViaLoad.MaxNumberOfItemsToReturn=maxNumberOfItemsToReturn;
		}
		...
		/// <param name="prefetchPathToUse">the PrefetchPath which defines the graph of objects to fetch as well</param>
		protected virtual void InitClassFetch(System.Int32 statusId, IValidator validator, IPrefetchPath prefetchPathToUse)
		{
			OnInitializing();
			base.Validator = validator;
			InitClassMembers();
			base.Fields = CreateFields();
			bool wasSuccesful = Fetch(statusId, prefetchPathToUse, null, null);
		...
		/// is set to false, the fields contained in excludedIncludedFields are kept in the query, the rest of the fields in the query are excluded.</param>
		/// <returns>True if succeeded, false otherwise.</returns>
		private bool Fetch(System.Int32 statusId, IPrefetchPath prefetchPathToUse, Context contextToUse, ExcludeIncludeFieldsList excludedIncludedFields)
		{
			try
			{
				OnFetch();
				IDao dao = this.CreateDAOInstance();
				base.Fields[(int)StatusFieldIndex.StatusId].ForcedCurrentValueWrite(statusId);
		...
		{
			return DAOFactory.CreateStatusDAO();
		}
		...
		{
			return new StatusEntityFactory();
		}
		...
		public static IPrefetchPathElement PrefetchPathLoad
		{
			get
			{
				return new PrefetchPathElement(new Reliant.RenuOil.DAL.CollectionClasses.LoadCollection(),
					(IEntityRelation)GetRelationsForField("Load")[0], (int)Reliant.RenuOil.DAL.EntityType.StatusEntity, (int)Reliant.RenuOil.DAL.EntityType.LoadEntity, 0, null, null, null, "Load", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.OneToMany);
			}
		}
		...
		/// for this entity. Add the object returned by this property to an existing PrefetchPath instance.</summary>
		/// <returns>Ready to use IPrefetchPathElement implementation.</returns>
		public static IPrefetchPathElement PrefetchPathEmployeeCollectionViaLoad
		{
			get
			{
				...
				return new PrefetchPathElement(new Reliant.RenuOil.DAL.CollectionClasses.EmployeeCollection(), intermediateRelation,
					(int)Reliant.RenuOil.DAL.EntityType.StatusEntity, (int)Reliant.RenuOil.DAL.EntityType.EmployeeEntity, 0, null, null, GetRelationsForField("EmployeeCollectionViaLoad"), "EmployeeCollectionViaLoad", SD.LLBLGen.Pro.ORMSupportClasses.RelationType.ManyToMany);
			}
		}
		...
		public virtual System.Int32 StatusId
		{
			...
			set	{ SetValue((int)StatusFieldIndex.StatusId, value, true); }
		}
		...
		public virtual System.String StatusName
		{
			...
			set	{ SetValue((int)StatusFieldIndex.StatusName, value, true); }
		}
		...
		/// <summary> Retrieves all related entities of type 'EmployeeEntity' using a relation of type 'm:n'.</summary>
		/// <remarks>This property is added for databinding conveniance, however it is recommeded to use the method 'GetMultiEmployeeCollectionViaLoad()', because 
		/// this property is rather expensive and a method tells the user to cache the result when it has to be used more than once in the same scope.</remarks>
		public virtual Reliant.RenuOil.DAL.CollectionClasses.EmployeeCollection EmployeeCollectionViaLoad
		{
			get { return GetMultiEmployeeCollectionViaLoad(false); }
		}

		/// <summary> Gets / sets the lazy loading flag for EmployeeCollectionViaLoad. When set to true, EmployeeCollectionViaLoad is always refetched from the 
		/// persistent storage. When set to false, the data is only fetched the first time EmployeeCollectionViaLoad is accessed. You can always execute
		/// a forced fetch by calling GetMultiEmployeeCollectionViaLoad(true).</summary>
		[Browsable(false)]
		public bool AlwaysFetchEmployeeCollectionViaLoad
		{
			get	{ return _alwaysFetchEmployeeCollectionViaLoad; }
			set	{ _alwaysFetchEmployeeCollectionViaLoad = value; }
		}
				
		/// <summary>Gets / Sets the lazy loading flag if the property EmployeeCollectionViaLoad already has been fetched. Setting this property to false when EmployeeCollectionViaLoad has been fetched
		/// will clear the EmployeeCollectionViaLoad collection well. Setting this property to true while EmployeeCollectionViaLoad hasn't been fetched disables lazy loading for EmployeeCollectionViaLoad</summary>
		[Browsable(false)]
		public bool AlreadyFetchedEmployeeCollectionViaLoad
		{
			get { return _alreadyFetchedEmployeeCollectionViaLoad;}
			set 
			{
				if(_alreadyFetchedEmployeeCollectionViaLoad && !value && (_employeeCollectionViaLoad != null))
				{
					_employeeCollectionViaLoad.Clear();
				}
				_alreadyFetchedEmployeeCollectionViaLoad = value;
			}
		}
